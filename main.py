...
def find_restaurants(lat, long, radius, placeid, day, hour):
    ...
    occupancy_map = get_current_crowd(list(st), placeid, GOOGLE_API_KEY, day, hour, time_zone)
...
def find_store(lat, long, radius, placeid, day, hour):
...
def analytics():
    ...
    if query_type == "restaurant":
        return find_restaurants(lat, long, distance, placeid, day, hour)
    else:
        return find_store(lat, long, distance, placeid, day, hour)
